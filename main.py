  ...
  #      print("you asked for all points ... ")
   #     all_initial_points = [np.array([130, 30]), np.array(
    #        [50, 20]), np.array([200, 15]), np.array([310, 50])]
     3   shapes_dir = os.path.join(data_dir, "scale", object_list[class_nb])
## >>>>>>> 72163a99a99018d85da8bfbfa3437b173cdbee04
        shapes_list = list(glob.glob(shapes_dir+"/*"))
        for myobject_nb in range(len(shapes_list)):
            test_optimization(network_model, network_name, class_nb, myobject_nb, all_initial_points, obj_class_list,
